cfg_get() {
    _cg_key="$1" _cg_default="$2" _cg_val
    _cg_val=$(ksud module config get "$_cg_key" 2>/dev/null) || \
        _cg_val=$(cat "$YURIKEY_CONFIG_DIR/$_cg_key.val" 2>/dev/null)
...
cfg_set() {
    ...
    ksud module config set "$_cs_key" "$_cs_val" 2>/dev/null || {
        ...
        printf '%s' "$_cs_val" > "$YURIKEY_CONFIG_DIR/$_cs_key.val"
    }
...
cfg_delete() {
    ...
    ksud module config delete "$_cd_key" 2>/dev/null || {
        rm -f "$YURIKEY_CONFIG_DIR/$_cd_key.val" 2>/dev/null
    }
    unset _cd_key
}
